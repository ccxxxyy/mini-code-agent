...
> 当前版本 v1.1.0，1379 个测试全部通过（1 skipped）。
...
- 重型目录软链：`worktree_symlink_dirs` 可配列表（默认 .venv/node_modules/vendor，空列表禁用）自动链接进 worktree——Agent 免重装依赖；Windows 符号链接失败回退 junction；删除前先断开链接防误删主仓库

**验证**：15 个真实 git 仓库集成测试（创建/脏树保护/合并/冲突/链接配置/空列表禁用/删除安全）
...
| 测试 | 1379 个测试全部通过（1 skipped，约 100 秒，零网络依赖），单元 64 文件 + 集成 5 文件 |
...
| 长记忆自动化 | PRE_LLM hook 自动注入记忆（LLM 选择性召回与主调用并行预取，首 token 零延迟）+ SESSION_END hook 自动提取偏好 + 启动后台整固节律（时间+会话数双门槛无感合并）——用户无感知的跨会话记忆 |
...
| 对话分叉/回滚 | `/undo` 回滚 N 轮 + `/fork` 分叉新会话原线保留（P26）——CC 无此能力（服务端历史不可操作），本地自持有 Conversation 的差异化优势；P27 进一步支持操作级撤销（undo 连文件一起恢复）；选择性恢复 --code-only/--conv-only（仅代码/仅对话）+ 快照保留轮数 undo_keep_turns 配置（tech-notes §113） |
