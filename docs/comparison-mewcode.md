...
- 用户可以审计全部代码（4500 行 vs 15000 行 + SDK 黑盒）
...
| 测试 | **430 测试，83.56% 覆盖率，fail_under=80** | 27 个测试文件，覆盖率未知 |
...
| 技术笔记 | tech-notes.md（35 节，设计决策记录） | 无 |
...
| | mini | mewcode |
|---|---|---|
| 工具执行时机 | 等 LLM 完整响应后再执行 | **边流式边执行**——工具调用事件一完成就提交 |

**差距**：LLM 一次返回 3 个工具调用时，mini 等全部解析完才执行；mewcode 第一个工具调用解析完就立即执行，第二个还在流式传输时第一个已经在跑了。
...
| | mini | mewcode |
|---|---|---|
| 输入框文件引用 | ❌ 需要说"帮我读 xxx" | ✅ `@README.md` 自动内联文件内容 |

**差距**：用户想让 LLM 看某个文件，要浪费一轮工具调用。
...
| | mini | mewcode |
|---|---|---|
| 大文件处理 | 直接塞进对话 | **>50K 字符的结果存磁盘，对话只留摘要** |

**差距**：这是当前 mini 唯一的高严重度遗留问题（tech-notes 34.3 ③）。用户说"读 spec.md"（大文件），内容全进对话→触发压缩→LLM 忘了读过→重读→再压缩→循环烧 token。
...
| | mini | mewcode |
|---|---|---|
| 压缩后状态恢复 | ❌ 压缩就丢了 | ✅ 摘要后重新挂载最近读过的文件列表和激活的 skill |

**差距**：压缩后 LLM 不知道自己读过哪些文件，导致重读。
...
| 优先级 | 编号 | 增强项 | 解决什么 | 工作量 |
|---|---|---|---|---|
| 🔴 P0 | 4.1 | 大工具结果溢写磁盘 | 压缩-重读膨胀（唯一高严重度遗留） | 1 天 |
| 🔴 P0 | 4.2 | 压缩后恢复（已读文件列表） | 同上 | 半天 |
