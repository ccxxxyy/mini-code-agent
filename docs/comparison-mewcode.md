...
| 测试 | **1063 测试，80%+ 覆盖率，fail_under=80** | 27 个测试文件，覆盖率未知 |
...
- 21 个内置工具（原 8 + tool_search + mcp_call + send_message + wait_message + ask_user + exit_plan_mode + task_create/get/list/update + load_skill + install_skill + synthetic_output）
