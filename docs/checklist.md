...
- [x] `core/subagent.py` SubAgent 初始化：无 context_summary 时追加反幻觉提示（有时不触发——已有真实摘要）
- [x] 覆盖所有 agent 类型（含自定义 .md）——注入点在 agent 类型 prompt 之后而非逐类型修改
- [x] 诚实边界文档化：prompt 守则是提示非强制，LLM 仍可能违反

## /session list 分页 检查项

- [x] `_SESSION_LIST_LIMIT = 20` 模块常量（不进配置——`--all` 已是出口，无配置价值）
- [x] list 参数解析改 token 扫描：`--all` / `--page N` / `--tag <name>`，任意顺序组合
- [x] >20 条默认截断 + 尾行双语提示 `Page N/M (总数) —— --page N+1 下一页 / --all 全部`；≤20 条无尾行
- [x] `--page N` 真分页：第 N 页显示第 (N-1)*20+1 ~ N*20 条；末页无下一页提示；页码超范围报错并提示总页数；非法页码值忽略回退第 1 页
- [x] `--all` 优先于 `--page`
- [x] usage 行补 `[--page N] [--all]`
- [x] 7 个新测试（截断+尾行 / --all 全量 / ≤20 无尾行 / --tag 组合 / --page 2 末页 / 超范围 / --all 优先），1182→1189
