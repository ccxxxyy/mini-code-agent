...
│   ├── unit/                        # 64 unit test files, 1166 tests
│   │   ├── test_agent_loop.py
│   │   ├── test_permissions.py
...
class Terminal:
    ...
    async def _prompt_protected(session, message: str) -> str:
        """prompt_async wrapped in patch_stdout: concurrent output during
        the input wait (trace lines from parallel tools) reroutes above
        the prompt instead of printing into the input line. Used by
        confirm/ask_yes_no/ask_structured. Falls back to a bare prompt
        when the stdout proxy cannot be built (no console)."""
        ...
