...
| 颜色 | 提示符 `>` 跟随 `theme.primary`；输入文字 bold 亮浅蓝 `theme.user_input`；输入行上下有同色横线（`terminal.py` `_input_rule`） |
| 关闭方法 | 不可关闭（关了没法输入） |
...
| `[sandbox] ...` 沙箱状态提示 | `app.py` 启动 | 仅当 sandbox=true 且后端真正不可用时（如 Linux 既无 bwrap 也无 unshare）；Windows 非管理员不再打印（无文件保护的限制仅在 config-guide 中文档化）；后端正常时不显示 |
