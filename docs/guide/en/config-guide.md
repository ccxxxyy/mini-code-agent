...
2. Sensitive filename patterns (`.env`/`.env.*`/`*.pem`/`*.key`/`id_rsa*`/`id_ed25519*`/`id_ecdsa*`/`id_dsa*`/`credentials*`/`*secret*`/`*.p12`/`*.pfx`/`*.ppk`/`*.jks`/`*.keystore`/`.npmrc`/`.pypirc`/`.netrc`/`_netrc`/`.git-credentials`/`.htpasswd`/`authorized_keys`, 22 kinds in total; plus 2 directory-aware patterns `.docker/config.json`/`.kube/config` — the bare names config.json/config are too generic and would flag ordinary project files, so they match as parent-dir/name pairs) → hard reject (blocked even inside the project); `.env.example`/`.env.sample`/`.env.template` are exempt
