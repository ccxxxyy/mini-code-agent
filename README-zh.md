...
工作目录就是你运行命令时所在的目录——和终端模式完全一样。会话自动持久化：每轮自动保存，服务器崩溃/被杀后重启自动恢复本项目最新未关闭会话（正常关闭后重启从新会话开始，可 `/session load` 手动恢复）。
...
**全部阶段已完成，1379 个测试全绿（1 skipped）。** 18 项需求的逐条实现证据见 [docs/capabilities.md](docs/capabilities.md)。
...
loading = "dispatch"         # "eager"（全部注册）| "native"（Anthropic 原生延迟）| "dispatch"（按需搜索+调用）
```
